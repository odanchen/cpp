...
int main()
{
    ...
    double p = (distance(side1, side2) + distance(side1, side3) + distance(side2, side3))/2;
    double ans = sqrt(p * (p - distance(side1, side2)) * (p - distance(side1, side3))
                      * (p - distance(side2, side3)));
