...
int main()
{
    Tlong results[200], one;
    int teams, disciplines, winning_team = 0;
    cin >> teams >> disciplines;
    one.number[nmax - 1] = 1;
    one.len = 1;
    for (int i = 0; i < teams; i++)
    {
        ...
        {
            Tlong cur_num;
            input_number(cur_num);
            results[i] = multiply(results[i], cur_num);
        }
    }
    for (int i = 0; i < teams; i++)
        if (compare(results[i], results[winning_team]) == 1) winning_team = i;
    cout << winning_team + 1;
}
